...
<div class="x-body layui-anim layui-anim-up">
  <form class="layui-form">
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline">
        <select class="layui-select" id="room_id" name="room_id" lay-filter="room" lay-verify="required" disabled>
          <option value="">请选择</option>
        </select>
      </div>
    </div>
...
<script>
  layui.use(['form', 'layer', 'laydate'], function () {
    ...
    function getLeaseAjax () {
      var lease_id = window.parent.document.getElementById('update_lease_id').value;
      $.ajax({
        url: 'getLease?leaseId='+lease_id,
        type: 'get',
        dataType: 'json',
        success:function (data) {
          $("#lease_id").val(data.data.lease_id);
          $("#customer_id").val(data.data.customer_id);
          $("#room_id").val(data.data.room_id);
          // $("#lease_team").val(data.data.lease_team);
          $("#rent").val(data.data.rent);
          $("#sing_time").val(data.data.sing_time);
          $("input[name=lease_team][value='1']").attr("checked", data.data.lease_team == 1 ? true : false);
          $("input[name=lease_team][value='2']").attr("checked", data.data.lease_team == 2 ? true : false);
          $("input[name=lease_team][value='3']").attr("checked", data.data.lease_team == 3 ? true : false);
          form.render();
        }
      });
    }
